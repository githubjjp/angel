package com.pingan.angel.auth.endpoint;
...
public class AngelTokenEndpoint {
	...
	public Result<Boolean> logout(@RequestHeader(value = HttpHeaders.AUTHORIZATION, required = false) String authHeader) {
		if (StringUtils.hasText(authHeader)) {
			...
			if (accessToken == null || StrUtil.isBlank(accessToken.getValue())) {
				return new Result<>(false, "退出失败，token 为空");
			}
	...
	public Result<Boolean> delToken(@PathVariable("token") String token, @RequestHeader(required = false) String from) {
	...
	public Result tokenList(@RequestBody Map<String, Object> params, @RequestHeader(required = false) String from) {
		if (StrUtil.isBlank(from)) {
			return null;
		}
		...
		for (String page : pages) {
			...
			if (authentication instanceof UsernamePasswordAuthenticationToken) {
				...
				if (authenticationToken.getPrincipal() instanceof AngelLoginUser) {
					AngelLoginUser user = (AngelLoginUser) authenticationToken.getPrincipal();
